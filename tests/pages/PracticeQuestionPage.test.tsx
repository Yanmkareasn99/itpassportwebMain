...
vi.mock("../../src/components/QuestionMedia", () => ({
  QuestionImage: () => null,
  AnswerChoiceContent: () => <span>Choose answer</span>,
}));
...
it("keeps an answer retryable after a database failure and prevents duplicate clicks while saving", async () => {
  ...
  expect(
    (screen.getByRole("button", { name: /Choose answer/ }) as HTMLButtonElement)
      .disabled,
  ).toBe(false);
  fireEvent.click(screen.getByRole("button", { name: "Answer" }));
  await act(async () => {
