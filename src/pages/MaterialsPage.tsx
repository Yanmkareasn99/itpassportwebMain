      ...
      <div className="app-shell space-y-6">
        <section className="bg-gradient-to-r from-sky-50 to-blue-100/60 dark:from-slate-900 dark:to-slate-800 rounded-2xl border border-sky-100 dark:border-slate-700 p-5 sm:p-7">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-800 dark:text-slate-100 mb-2">{text.title}</h2>
                <p className="text-sm text-gray-600 dark:text-slate-300 leading-6 max-w-2xl">{text.lead}</p>
              </div>
            </div>
            <button
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {materials.map(item => {
            ...
            const colorClass =
              item.color === 'emerald'
                ? 'bg-emerald-50 text-emerald-600 border-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-700'
                : item.color === 'amber'
                ? 'bg-amber-50 text-amber-600 border-amber-100 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-700'
                : 'bg-blue-50 text-blue-600 border-blue-100 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-700';

            return (
              <article key={item.title} className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-5 flex flex-col min-h-[230px]">
                <div className={`w-11 h-11 rounded-xl border flex items-center justify-center mb-4 ${colorClass}`}>
                  <Icon className="w-5 h-5" />
                </div>
                <h3 className="font-bold text-gray-800 dark:text-slate-100 mb-2">{item.title}</h3>
                <p className="text-sm text-gray-500 dark:text-slate-300 leading-6 flex-1">{item.description}</p>
                ...
                <div className="grid grid-cols-2 gap-2 mt-4">
                  <button className="px-3 py-2 rounded-xl bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 text-xs font-semibold hover:bg-gray-200 dark:hover:bg-slate-700 transition">
                    {text.open}
                  </button>
