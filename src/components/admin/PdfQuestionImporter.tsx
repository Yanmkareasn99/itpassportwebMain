...
function questionProblem(question: PdfImportQuestion) {
  ...
  if (
    ...
    !question.choices.some((choice) => choice.label === question.correctChoice)
  ) {
    return "correct";
  }
  return "";
...
function nextChoiceLabel(choices: PdfImportChoice[]) {
  ...
  return (
    [...labels].find(
      (label) => !choices.some((choice) => choice.label === label),
    ) ?? String(choices.length + 1)
  );
}

async function uploadQuestionImage(
  question: PdfImportQuestion,
  examKey: string,
) {
...
export default function PdfQuestionImporter({
  ...
  const imageSizeMb = useMemo(
    () =>
      questions.reduce(
        (sum, question) => sum + question.imageDataUrl.length * 0.75,
        0,
      ) /
      1024 /
      1024,
    ...
    setQuestions((current) =>
      current.map((question, currentIndex) => {
        ...
        return {
          ...
          choices: [
            ...question.choices,
            { label, text: label, sortOrder: question.choices.length + 1 },
          ],
        };
      }),
    );
    ...
    setQuestions((current) =>
      current.map((question, currentIndex) => {
        ...
        return {
          ...question,
          correctChoice:
            question.correctChoice === removed.label
              ? ""
              : question.correctChoice,
    ...
    try {
      ...
      setError(
        processError instanceof Error
          ...
          : translate(language, "adminPage.pdfProcessingFailed"),
      );
    } finally {
      setProcessing(false);
    }
  ...
  async function handleImport() {
    ...
    try {
      for (let index = 0; index < questions.length; index += 1) {
        ...
        setProgress(
          translate(language, "adminPage.pdfImportProgress", {
            current: index + 1,
      ...
      setProgress("");
      setSuccess(
        translate(language, "adminPage.pdfImportComplete", {
      ...
      setError(
        /question_import_staging|question-images|bucket not found/i.test(
          message,
        )
          ? translate(language, "adminPage.pdfMigrationRequired")
    ...
    <section className="rounded-2xl border border-violet-200 bg-white p-5 shadow-sm">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {translate(language, "adminPage.pdfImportDescription")}
          </p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <label className="block text-xs font-semibold text-gray-600">
          ...
          <input
            value={examKey}
            onChange={(event) =>
              setExamKey(event.target.value.replace(/\s/g, ""))
            }
            placeholder="2026B"
            className="mt-1 w-full rounded-xl border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-300"
          />
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {processing
            ? progress || translate(language, "adminPage.pdfProcessing")
            : translate(language, "adminPage.pdfProcess")}
        </button>
        <p className="text-xs text-gray-500">
          {translate(language, "adminPage.pdfLocalProcessing")}
        </p>
      </div>
          ...
          <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="text-sm font-bold text-gray-800">
                {translate(language, "adminPage.pdfReviewCount", {
                  count: questions.length,
                })}
              </h3>
              <p
                className={`mt-0.5 text-xs ${invalidCount ? "text-amber-700" : "text-emerald-700"}`}
              >
                {invalidCount
                  ? translate(language, "adminPage.pdfReviewProblems", {
                      count: invalidCount,
                    })
            ...
            >
              ...
              {importing
                ? progress
                : translate(language, "adminPage.pdfImportButton")}
            </button>
          </div>
            ...
            {questions.map((question, questionIndex) => {
              ...
              return (
                <div
                  ...
                  className={`overflow-hidden rounded-xl border ${problem ? "border-amber-200" : "border-gray-200"}`}
                >
                  <button
                    onClick={() =>
                  ...
                  >
                    ...
                    <span className="w-14 shrink-0 text-sm font-bold text-gray-700">
                      問 {question.number}
                    </span>
                    <span className="min-w-0 flex-1 truncate text-xs text-gray-500">
                      {problemText ||
                        `${question.choices.length} ${translate(language, "adminPage.choices")} · PDF ${question.sourcePages.join(", ")}`}
                    </span>
                    {expanded ? (
